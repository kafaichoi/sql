...
defmodule SQL.Adapters.ANSI do
  ...
  def token_to_string({tag, _, [{:parens, _, _} = value]}, mod)
      when tag in ~w[integer float update]a do
  ...
  def token_to_string({tag, _, value}, mod)
      when tag in ~w[select from fetch limit where order offset group having with join by distinct create type drop insert alter table add into delete update start grant revoke set declare open close commit rollback references recursive]a do
    "#{mod.token_to_string(tag)} #{mod.token_to_string(value)}"
  end
  ...
  def token_to_string({tag, _, [left, [{:all = t, _, right}]]}, mod)
      when tag in ~w[union except intersect]a do
  ...
  def token_to_string({tag, _, [left, right]}, mod)
      when tag in ~w[:: [\] <> <= >= != || + - ^ * / % < > = like ilike as union except intersect between and or on is not in cursor for to]a do
    "#{mod.token_to_string(left)} #{mod.token_to_string(tag)} #{mod.token_to_string(right)}"
  end
  ...
  def token_to_string({tag, _, values}, mod)
      when tag in ~w[not all between symmetric absolute relative forward backward on in for without]a do
